...
5. Если `config` невалиден (отсутствует (для ROT-13 и Цезаря), присутствует (для Атбаш) или неправильный `Y`, неправильный формат, посторонние символы и т.д.), приложение передает соответствующее сообщение в `process.stderr` и прoцесс завершается с кодом, отличным от 0 **плюс 20 баллов**
